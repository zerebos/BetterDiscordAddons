    ...
    <ul class="member-perms \${root} \${rolesList} \${endBodySection}"></ul>
        ...
        createModal(title, displayRoles, referenceRoles, isOverride = false) {
            ...
            for (const r in displayRoles) {
                ...
                if (isOverride) item.querySelector(".role-name").textContent = displayRoles[role].type == 0 ? referenceRoles[role].name : user.username;
                else item.querySelector(".role-name").textContent = referenceRoles[role].name;
