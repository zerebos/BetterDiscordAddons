...
class Plugin {
	...
	getMessageColor(message) {
		...
		let msg = ReactUtilities.getReactKey({node: message, key: "message", whiteList: {
			"props":true,
			"children":true,
			"memoizedProps":true,
			"memoizedState":true,
	...
	colorizeTyping() {
		...
		var typingUsers = ReactUtilities.getReactKey({node: document.querySelector('.typing').parentElement, key: "typingUsers", whiteList: {
			"memoizedState": true,
			"child": true,
			"sibling": true
		}})
		if (Object.getOwnPropertyNames(typingUsers).length === 0) return;
	...
	colorizeVoice() {
	    ...
	    document.querySelectorAll(".draggable-3SphXU").forEach((elem, index) => {
	        var user = ReactUtilities.getReactKey({node: elem, key: "user", whiteList: {
				"props": true,
				"children": true,
				"memoizedProps": true
			}})
	...
	colorizeMentions() {
	    ...
	    document.querySelectorAll(".message-group .message").forEach((elem, index) => {
	    	var messages = ReactUtilities.getReactKey({node: elem.parentElement.parentElement, key: "messages"})
	    	...
	    	elem.querySelectorAll('.message-text > .markup > .mention').forEach((elem, index) => {
				var isUserMention = ReactUtilities.getReactKey({node: elem, key: "name", value: "bound handleUserContextMenu", whiteList: {
					"memoizedProps": true,
					"onContextMenu": true
				}})
	...
	colorizePopout() {
        ...
        var user = ReactUtilities.getReactKey({node: popout, key: "user", whiteList: {
			"props": true,
			"children": true,
			"memoizedProps": true
		}})
	...
	colorizeModal() {
        ...
        var user = ReactUtilities.getReactKey({node: modal, key: "user", whiteList: {
			"props": true,
			"children": true,
			"memoizedProps": true
		}})
	...
	colorizeAuditLog() {
	    ...
	    document.querySelectorAll('.userHook-DFT5u7').forEach((elem, index) => {
	        var user = ReactUtilities.getReactKey({node: elem.parentElement, key: "discriminator", value: elem.querySelector(".discrim-xHdOK3").textContent.slice(1), returnParent: true, whiteList: {
				"props": true,
				"children": true,
				"memoizedProps": true,
				"child": true,
